...
INSERT INTO external_services (`service_name`, `request_type`, `request_options`,`request`, `status`, `created_at`, `updated_at`) VALUES
  ...
  '{ "URL": "https://172.16.111.5:4301/azam/airtel/ussdpush", "METHOD": "POST", "REJECT_UNAUTHORIZED": false, "AGENT_OPTIONS": { "securityOptions": "TLSv1_method", "CERT": "/home/atharv/Airtel_push_api.crt", "KEY": "/home/atharv/Airtel_push_api.key", "PASS_PHRASE": "PUSH_API@123" } }',
  ...
  ('ValidateTigoTokenInternal', 'object',
   '{ "URL": "https://172.16.111.5:4101/tigo/ValidateToken", "REJECT_UNAUTHORIZED": false, "METHOD": "POST", "HEADERS": { "Content-type": "application/json" } }',
  ...
  '{ "URL": "https://172.16.111.5:4101/tigo/InitiateTransaction", "REJECT_UNAUTHORIZED": false, "METHOD": "POST" }',
  '`{ "PartnerCode": "${options.PartnerCode}", "CustomerMSISDN": "${options.CustomerMSISDN}", "ReferenceMSISDN": "${options.ReferenceMSISDN}", "Amount": "${options.Amount}" }`',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ...
  ('FetchAirtelTransactions', 'object', '{ "URL": "https://172.16.111.5:4301/azam/airtel/fetchTransactions", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }', '{}', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchHalotelTransactions', 'object',
  '{ "URL": "https://172.16.111.5:4201/azam/halotel/fetchTransactions", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchTigoTransactions', 'object',
  '{ "URL": "https://172.16.111.5:4201/azam/tigo/fetchTransactions", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchVodacomTransactions', 'object',
  '{ "URL": "https://172.16.111.5:4201/azam/vodacom/fetchTransactions", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchZantelTransactions', 'object',
  '{ "URL": "https://172.16.111.5:4201/azam/zantel/fetchTransactions", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ...
  '{ "URL": "https://172.16.111.5:4300/azam/confirm/payment", "METHOD": "POST", "JSON": true, "REJECT_UNAUTHORIZED": false }',
  '`{ "apiPath": "${options.apiPath}", "transactionstatus": "${options.transactionstatus}", "message": "${options.message}", "operator": "${options.operator}", "reference": "${options.reference}", "utilityref": "${options.utilityref}", "amount": "${options.amount}", "transid": "${options.transid}", "msisdn": "${options.msisdn}" }`',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ...
  '{ "URL": "https://172.16.111.5:4301/azam/airtel/fetchTransactionsCount", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchHalotelTransactionsCount', 'object',
  '{ "URL": "https://172.16.111.5:4301/azam/halotel/fetchTransactionsCount", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchTigoTransactionsCount', 'object',
  '{ "URL": "https://172.16.111.5:4301/azam/tigo/fetchTransactionsCount", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchVodacomTransactionsCount', 'object',
  '{ "URL": "https://172.16.111.5:4301/azam/vodacom/fetchTransactionsCount", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

  ('FetchZantelTransactionsCount', 'object',
  '{ "URL": "https://172.16.111.5:4301/azam/zantel/fetchTransactionsCount", "METHOD": "POST", "REJECT_UNAUTHORIZED": false }',
  '{}',
  'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
INSERT INTO `external_services` (`service_name`, `request_type`, `request_options`, `request`, `status`, `created_at`, `updated_at`)
VALUES
(X'436865636B55736572466F72417A616D545652657175657374', X'737472696E67', '{\"URL\": \"http://mq.azammedia.com/mqsapiws/Service.asmx\",\"METHOD\": \"POST\",\"HEADERS\": {\"Content-type\": \"text/xml\"},\"QS\": {\"WSDL\": \"\"}, \"REJECT_UNAUTHORIZED\": false, \"INSECURE\": false}', '`<Envelope xmlns=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ns1=\"http://tempuri.org/\"><Header><ns1:MQUserNameToken><ns1:User_id>AzamPay</ns1:User_id><ns1:Password>@azAmpay!17</ns1:Password><ns1:ExternalPartyName>AZAMPAY</ns1:ExternalPartyName></ns1:MQUserNameToken></Header><Body><ns1:GetCustomerInfo><ns1:CustomerInfoXML><![CDATA[<REQUESTINFO><KEY_NAMEVALUE><KEY_NAME>SMARTCARD</KEY_NAME><KEY_VALUE>${options.referenceNumber.senderMsisdn}</KEY_VALUE></KEY_NAMEVALUE></REQUESTINFO>]]></ns1:CustomerInfoXML><ns1:ReferenceNo>${options.referenceNo}</ns1:ReferenceNo></ns1:GetCustomerInfo></Body></Envelope>`', X'616374697665', '2020-07-15 13:05:55', '2020-07-20 12:38:54'),
(X'5061796D656E744D7153657276696365', X'737472696E67', '{ \"URL\": \"http://mq.azammedia.com/mqsapiws/Service.asmx\", \"METHOD\": \"POST\", \"HEADERS\": { \"Content-type\": \"text/xml\" }, \"QS\": { \"WSDL\": \"\" }, \"REJECT_UNAUTHORIZED\": false, \"INSECURE\": false }', '`<Envelope xmlns=\'http://schemas.xmlsoap.org/soap/envelope/\' xmlns:ns1=\'http://tempuri.org/\'> <Header><ns1:MQUserNameToken> <ns1:User_id>AzamPay</ns1:User_id> <ns1:Password>@azAmpay!17</ns1:Password><ns1:ExternalPartyName>AZAMPAY</ns1:ExternalPartyName> </ns1:MQUserNameToken> </Header><Body> <ns1:MakePayment> <ns1:MakePaymentXML><![CDATA[<REQUESTINFO><KEY_NAMEVALUE><KEY_NAME>SMARTCARD</KEY_NAME><KEY_VALUE>${options.smartCardNo}</KEY_VALUE></KEY_NAMEVALUE><PAYMENTINFO><AMOUNT>${options.amount}</AMOUNT><PAYMODE>CA</PAYMODE><RECEIPTNO>${options.reciptNo}</RECEIPTNO><PAYMENTDATE>${options.paymentDate}</PAYMENTDATE><OPENITEM>DEFAULT</OPENITEM><REMARKS>${options.remarks}</REMARKS></PAYMENTINFO></REQUESTINFO>]]></ns1:MakePaymentXML> <ns1:ReferenceNo>${options.referenceNo}</ns1:ReferenceNo> </ns1:MakePayment></Body></Envelope>`', X'616374697665', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
